...
static/
  index.html         # editor page: top bar + 3-column main (collapsible MEDIA panel | center column: 9:16 stage filling height + timeline strip below it | selection-driven context panel: VIDEO/TEXT/CAPTIONS), per the north-star mockup layout
  editor.js           # UI state + API calls + DOM wiring (thin); owns the client-side TextPreset stand-in, MEDIA panel collapse state, and selection state driving which context-panel section (VIDEO/TEXT/CAPTIONS) is open; also wires transport step-back/step-forward buttons and global keyboard shortcuts (Left/Right nudge 0.1s, Up toggles play/pause), skipped while focus is in an input/textarea/select
...
- `static/preview.js` — `Preview.renderText(project, presets, timelineTime)` composites one `.text-block` div (may be multiline via `\n`, rendered with the stage CSS's existing `white-space: pre`) per visible block into `#overlay` (position/size scaled from the 1080x1920 canvas to the stage's actual pixel size; outline via `-webkit-text-stroke` or `box`/`box_color` background; `font-family`/`font-weight`/`font-style`/`text-decoration` set from `preset.font`/`bold`/`italic`/`underline`, added 2026-07-14); `Preview.currentTimelineTime()` exposes the last computed tick so editor.js can re-render immediately while paused. `Preview.play()`/`Preview.pause()` (added 2026-07-14) back the transport's explicit play/pause buttons and editor.js's Up-arrow toggle; `play()` keeps the old "restart from clip 0 if at the end" behavior. `#time`'s numeric readout is hidden via CSS (`stage.css`) since the timeline toolbar shows the time now, but the element still holds the value as `textContent` — editor.js's `nudgeTime`/`renderTimeline` still read it.
