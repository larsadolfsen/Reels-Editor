...
static/
  ...
  panel-media.js          # FILES/MEDIA context-panel section: media-library list (thumbnail, name, duration), click-to-select (extracted from editor.js 2026-07-20); hover-revealed pencil icon opens inline rename (Task 4, 2026-07-20)
  panel-video.js          # VIDEO context-panel section: trim/order/delete for the selected clip (extracted from editor.js 2026-07-20); also hosts the shared clampTrim() helper and the VIDEO-panel's TRIM/ORDER divider wiring, moved here from editor.js 2026-07-21 since panel-video.js/panel-video-box.js are its only consumers
  panel-text.js           # TEXT context-panel section: FONT/STYLES/BOX/TIME accordion orchestration + stage resize/move handlers (extracted from editor.js 2026-07-21); addTextBlockAndEdit() (wired to both the empty-state "+ Add text" button and the timeline's TEXT-row + button, Task 6 2026-07-21) creates the block, opens the panel, and calls Preview.enterTextEditMode(block.id) so the new block is immediately live-editable (contentEditable) on the stage
...
- `app/models.py` — Pydantic entities: `MediaItem(id, file_path, name, duration, has_audio)`, `Project`, `ProjectSummary`, `ClipLayer`, `VideoBoxLayer`, `TextPreset`, `FormatRun`, `TextBlockLayer`, `CaptionWord`, `CaptionTrack`, `new_id()`. Field-level detail lives under each owning feature below (media/import for `MediaItem`, project management for `Project`/`ProjectSummary`, timeline/video clips for `ClipLayer`, video boxes for `VideoBoxLayer`, text blocks for `TextPreset`/`FormatRun`/`TextBlockLayer`, captions for `CaptionWord`/`CaptionTrack`).
...
- `MediaItem(id, file_path, name, duration, has_audio)` in `app/models.py` — separates imported media library from timeline clip references. `name: str = ""` (Task 3, added 2026-07-20: optional custom media name; display falls back to `file_path` basename if empty). `has_audio: bool = True` (Phase 6, added 2026-07-20: whether ffprobe found an audio stream; defaults `True` so existing saved projects behave unchanged until re-probed) is consumed by the export pipeline (see below) to decide whether a clip needs synthesized silent audio. `display_name` property returns `name` if non-empty, else `file_path` basename.
...
- `static/clip-sequence.js` — `addClip()` (extracted from `editor.js` 2026-07-21) pushes a `MediaItem` into `project.media_library` alongside the `ClipLayer` (carrying `media_id`) added to `project.clips`, then calls `MediaPanel.render()`; wired to the `#add-clip` click listener.
- `static/editor.js` — `setPanelCollapsed(bool)` toggles the MEDIA panel's `#panel.collapsed` (72px icon rail) and persists to `localStorage` (`panelCollapsed`).
- `static/panel-media.js` — `MediaPanel.render()` (extracted from `editor.js` 2026-07-20): renders `project.media_library` as thumbnail+name+duration rows (`formatClipDuration`, mm:ss.s) into `#clip-list`, click-to-toggle a row's highlight via module-local `selectedMediaId` — purely local state, independent of timeline `selected` and with zero side effects on player/timeline/VIDEO panel; also wires `draggable`/`dragstart` for drag-to-timeline. Task 4 (2026-07-20): each row's name resolves via `m.name || m.file_path.split(/[\\/]/).pop()`; a hover-revealed pencil icon button (`.clip-actions`) swaps the `.clip-name` label for a text `<input>` — Enter/blur commits (`m.name = value.trim(); saveProject(); render()`), Escape cancels and restores the label with no save. Task 5 (2026-07-20): each row also computes `count = project.clips.filter(c => c.media_id === m.id).length` fresh on every render; `count > 0` shows a `.clip-usage-chip` badge and disables the row's trash button with `title="used by N clip(s)"`; `count === 0` enables the trash button, which removes the item from `project.media_library` only (`saveProject()` + re-render, no confirmation, no cascade delete of `project.clips`).
...
- `static/panel-captions.js` — `defaultCaptionPreset(id)`/`ensureCaptionPreset(id)`/`ensureCaptionTrack()` (lazily creates/loads `project.captions` and its preset), `renderCaptionPreview()`, `renderCaptionPanel()` (the `#panel-captions` orchestrator: shows/hides the FONT/WEIGHT/STYLE/WORDS drill-downs vs the main view, delegates to each `CaptionPanel.render*()`), its accordion/divider wiring, and the `#caption-auto-btn` transcribe-click listener; extracted from `editor.js` 2026-07-21, pure move, zero behavior change.
