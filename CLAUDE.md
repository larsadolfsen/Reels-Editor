...
static/
  ...
  editor.js           # UI state + API calls + DOM wiring (thin); owns the client-side TextPreset stand-in, MEDIA panel collapse state, and selection state driving which context-panel section (VIDEO/TEXT/CAPTIONS) is open
...
- `static/preview.js` — `Preview.renderText(project, presets, timelineTime)` composites one `.text-block` div (may be multiline via `\n`, rendered with the stage CSS's existing `white-space: pre`) per visible block into `#overlay` (position/size scaled from the 1080x1920 canvas to the stage's actual pixel size; outline via `-webkit-text-stroke` or `box`/`box_color` background; `font-family`/`font-weight`/`font-style`/`text-decoration` set from `preset.font`/`bold`/`italic`/`underline`, added 2026-07-14); `Preview.currentTimelineTime()` exposes the last computed tick so editor.js can re-render immediately while paused.
