...
- New `static/text-panel-font-weight.js` (this project's convention is one component per file — a new file, not an addition to `text-panel-font-style.js`), following the Font Family row/drill-down pattern (`UI.settingsRow` + `UI.subPanelHeader`), with one deliberate difference: rows stay in ascending weight order (400→700 is itself meaningful, unlike font names) rather than pinning the current selection to the top, and each row shows a **live inline preview** — the block's actual heading text rendered in the current font family at that row's exact weight — instead of Font Family's hover-to-preview-on-stage (a preview visible for every option at once, without needing to hover each one), still with a checkmark on the current selection. Click-to-apply, same as Font Family.
