...
- **Task** = one component or service within a phase: one `UI.*` JS component, one CSS component file, one backend model/route change, one ASS-render addition, one service module. Small enough to hand to a single worktree subagent in one sitting. **Every phase's brainstorm must explicitly decompose it this way** — one task per component/service, not a handful of broad steps — and group the resulting tasks into batches that get *dispatched simultaneously*, not executed one after another in a single session. Getting this decomposition right is part of what the brainstorm is for, not an afterthought once a plan is already written (Phase 1's first draft got this wrong — its plan initially had accordion-extraction tasks phrased as "whatever's left after the previous task," which is inherently sequential; it was rewritten so each task edits only its own non-overlapping slice of the shared file, which is what actually makes them dispatchable in parallel).
- **Parallelize independent tasks — including ones that touch the same file.** Sharing a file (`static/index.html`, `static/editor.js`) is *not* by itself a reason to serialize two tasks. What matters is whether they edit the same region: if two tasks each add/remove a clearly separate, non-overlapping chunk of the same file, they're still parallel-safe — each worktree branches from the same commit, and the merges reconcile independently (a real conflict, if one comes up, gets resolved at merge time; it's not something to avoid by serializing tasks preemptively). Only tasks with a genuine *code* dependency (Task B's logic literally builds on Task A's exact diff, not just "both touch editor.js") are truly sequential. Run parallel-safe tasks via `superpowers:using-git-worktrees` + `superpowers:subagent-driven-development`, each in its own worktree dispatched at the same time as its batch-mates, merged back before the phase's visual checkpoint.
