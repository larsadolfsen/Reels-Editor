  ...
  if (typeof window !== "undefined") {
    ...
    document.getElementById("slice-action").addEventListener("click", async () => {
      ...
      if (selected && selected.type === "video-box" && Timeline.isBoxActiveAt(selected.item, t)) {
        ...
        await saveProject();
        const newBox = project.video_boxes.find((b) => b.id === newId);
        // Reselect the NEW (second) box, not the original: the original's window now ends at t
        // (end-exclusive), so it's no longer active/visible there — selecting it would leave the
        // stage showing an unselected box with no handles, and a second click at the same t would
        // silently fall through to slicing the main clip instead (the original is inactive at t).
        await onTimelineSelect({ type: "video-box", item: newBox });
        // onTimelineSelect's video-box branch calls VideoBoxPanel.render(), which itself calls
        // VideoBoxPreview.setSelectedVideoBox(...) — but selecting alone doesn't trigger a render
        // pass (same gap editor.js/panel-media.js already work around after their own
        // onTimelineSelect calls), so mount the handles at the current time explicitly:
